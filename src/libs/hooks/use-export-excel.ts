  ...
  const executeExport = useCallback(async () => {
    ...
    try {
      ...
      if (fetchResult.status === "success" && fetchResult.data) {
        ...
        // 열 너비 설정
        ws["!cols"] = headerCells.map(() => ({ wch: 20 }));
        ...
        for (let r = range.s.r; r <= range.e.r; r++) {
          for (let c = range.s.c; c <= range.e.c; c++) {
            ...
            if (r === 0) {
              ...
              // 데이터 행에 스타일 적용 (홀수/짝수 행 구분)
              ws[cellAddress].s = r % 2 ? oddRowStyle : dataStyle;
            }
          }
        }

        // 워크시트를 워크북에 추가
        XLSX.utils.book_append_sheet(wb, ws, "data-list");

        // 워크북을 Excel 파일로 변환 (스타일 포함)
        ...
        FileSaver.saveAs(data, formattedFileName + extension);
      } else {
